...
/****************************** MACROS ******************************/
// The least significant byte of the WORD2 is rotated to the end.
#define KE_ROTWORD2(x) (((x) << 8) | ((x) >> 24))
...
* AES - CBC
*******************/
int aes_encrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const WORD2 key[], int keysize, const BYTE iv[])
{
...
int aes_encrypt_cbc_mac(const BYTE in[], size_t in_len, BYTE out[], const WORD2 key[], int keysize, const BYTE iv[])
{
	BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
int aes_decrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const WORD2 key[], int keysize, const BYTE iv[])
{
	BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
void aes_encrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const WORD2 key[], int keysize, const BYTE iv[])
{
...
void aes_decrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const WORD2 key[], int keysize, const BYTE iv[])
{
...
int aes_encrypt_ccm(const BYTE payload[], WORD2 payload_len, const BYTE assoc[], unsigned short assoc_len,
                    const BYTE nonce[], unsigned short nonce_len, BYTE out[], WORD2 *out_len,
                    WORD2 mac_len, const BYTE key_str[], int keysize)
{
...
int aes_decrypt_ccm(const BYTE ciphertext[], WORD2 ciphertext_len, const BYTE assoc[], unsigned short assoc_len,
                    const BYTE nonce[], unsigned short nonce_len, BYTE plaintext[], WORD2 *plaintext_len,
                    WORD2 mac_len, int *mac_auth, const BYTE key_str[], int keysize)
{
...
// Substitutes a WORD2 using the AES S-Box.
WORD2 SubWORD2(WORD2 WORD2)
{
	unsigned int result;

	result = (int)aes_sbox[(WORD2 >> 4) & 0x0000000F][WORD2 & 0x0000000F];
	result += (int)aes_sbox[(WORD2 >> 12) & 0x0000000F][(WORD2 >> 8) & 0x0000000F] << 8;
	result += (int)aes_sbox[(WORD2 >> 20) & 0x0000000F][(WORD2 >> 16) & 0x0000000F] << 16;
	result += (int)aes_sbox[(WORD2 >> 28) & 0x0000000F][(WORD2 >> 24) & 0x0000000F] << 24;
	return(result);
}
...
void aes_key_setup(const BYTE key[], WORD2 w[], int keysize)
{
	int Nb=4,Nr,Nk,idx;
	WORD2 temp,Rcon[]={0x01000000,0x02000000,0x04000000,0x08000000,0x10000000,0x20000000,
	...
	for (idx = Nk; idx < Nb * (Nr+1); ++idx) {
		temp = w[idx - 1];
		if ((idx % Nk) == 0)
			temp = SubWORD2(KE_ROTWORD2(temp)) ^ Rcon[(idx-1)/Nk];
...
void AddRoundKey(BYTE state[][4], const WORD2 w[])
{
	BYTE subkey[4];
...
void aes_encrypt(const BYTE in[], BYTE out[], const WORD2 key[], int keysize)
{
	BYTE state[4][4];
...
void aes_decrypt(const BYTE in[], BYTE out[], const WORD2 key[], int keysize)
{
	BYTE state[4][4];
