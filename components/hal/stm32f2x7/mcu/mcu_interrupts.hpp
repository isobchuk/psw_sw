...
namespace stm32f217::drivers::interrupt {
...
template <const Number interruptNumber> class Interrupt final {
  static constexpr auto _RegNumber = cpp_register::reg_v<static_cast<InterruptT>(interruptNumber) / 32>;
  static constexpr auto _RegValue = cpp_register::reg_v<1UL, (static_cast<InterruptT>(interruptNumber) % 32)>;
...
public:
  ...
  inline void Enable() const {
    using namespace stm32f217::registers::nvic;
    NVIC->ISER[_RegNumber] |= NVIC_ISER::SETENA(_RegValue);
  }

  inline void Disable() const {
    using namespace stm32f217::registers::nvic;
    NVIC->ICER[_RegNumber] |= NVIC_ICER::CLRENA(_RegValue);
  }

  inline void PendingSet() const {
    using namespace stm32f217::registers::nvic;
    NVIC->ISPR[_RegNumber] |= NVIC_ISPR::SETPEND(_RegValue);
  }

  inline void PendingClear() const {
    using namespace stm32f217::registers::nvic;
    NVIC->ICPR[_RegNumber] |= NVIC_ICPR::CLRPEND(_RegValue);
  }

  [[nodiscard]] inline bool Status() const {
    using namespace stm32f217::registers::nvic;
    return (NVIC->ISER[_RegNumber] & NVIC_ISER::SETENA(_RegValue));
  }

  [[nodiscard]] inline bool PendingStatus() const {
    using namespace stm32f217::registers::nvic;
    return (NVIC->ISPR[_RegNumber] & NVIC_ISPR::SETPEND(_RegValue));
  }
};
